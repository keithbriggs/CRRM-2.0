...
# python3.11 CRRM_logger_test_00.py
...
def go():
    crrm_parameters = Parameters(
        ...
        move_stdev=10.0,
        author="Keith Briggs",
        layout_plot_fnbase="img/CRRM_logger_test_layout",
    ...
    for i in range(crrm_parameters.n_moves):
        logger.capture()
        deltas = crrm.params.move_stdev * crrm.rng.standard_normal(size=(n_ues, 2))
        crrm.move_ue_locations(move_indices, deltas)
        crrm.update()
    logger.plot(
        fields=("cqi", "mcs", "sinr", "se_Shannon", "tp"),
        fnbase="img/CRRM_logger_test",
        title=f"CRRM logger test n_cells={crrm_parameters.n_cells} n_ues={crrm_parameters.n_ues}",
        averages=("tp",),
